...
function logger(level, colors, trace, fn) {
  ...
  // Ensure chalk
  // When calling exported logger
  // chalk could be undefined.
  ...
  function log(type, msg) {
    ...
    // If an error was passed get just the message.
    if (err)
      args[0] = err.message || 'unknown error.';
    ...
    if (_trace) {
      const stack = getStackTrace(1, err);
      args.push(stack);
      origArgs.push(stack);
    }
...
Pargv.prototype.configure = function configure(index, options) {
  ...
  this.options.index = !isUndefined(index) ? index : this.options.index;
