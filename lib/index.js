...
module.exports = (function () {
  ...
  function Command(cmd, describe, pargv) {

    // Store the instance.
    this.pargv = pargv;

    // The primary command we are
    // creating options and validations
    // for.
    this.cmd = cmd;
  ...
  /**
   * Default
   * Create a default value for the specified key.
   *
   * @param {string} key the command or option key you are creating a default for.
   * @param {string} val the value of the default.
   */
  Command.prototype.default = function def(key, val) {
    this[key] = this[key] || {};
   ...
   *
   * @param {key} key the command or option you are describing.
   * @param {string} val the description.
   * @returns {Command}
   */
  Command.prototype.describe = function describe(key, val) {
    this[key] = this[key] || {};
    this[key].describe = val;
    return this;
  };

  /**
   * Alias
   * Adds alias(s) to the command.
   * @param {string} key the command or option you are aliasing.
   * @param {string} val an alias or array of aliases.
   * @returns {Command}
   */
  Command.prototype.alias = function alias(key, val) {
  ...
  /**
   * Default
   * Creates a default for an option
   *
   * @param {key} key the key name of the option or command.
   * @param {string} val the option value.
   * @returns {Command}
   */
  Command.prototype.option = function option(key, val) {

  };

  Command.prototype.require = function requires() {

  };

  /**
   * Done
   * Exits command builder returing Pargv instance.
   */
  Command.prototype.done = function done() {
    return this.pargv;
  };
  ...
  function Pargv() {
    ...
    Object.defineProperties(this, {
      ...
      chalk: { enumerable: false, writable: true, value: chalk },
      cli: {
        enumerable: false, writable: true, value: {
          // $0 will be replaced w/ exec command.
          // you can also use <exec>
          usage: 'usage: $0 <cmd> [opt]'
        }
      }
    })
  ...
  Pargv.prototype.configure = function configure(index, options) {
    ...
    if (this.options.colors === false)
      chalk = this.chalk = new Chalk({ enabled: false });
  ...
  /**
   * Version
   * Stores the version for the cli.
   * @param {string} [ver] the version of the app/cli.
   * @returns {Pargv}
   */
  Pargv.prototype.version = function version(ver) {
    ver = ver || pkg.version;
    this.cli.version = ver;
    return this;
  };

  /**
   * Usage
   * Adds usage description to the cli.
   * @param {string} desc the usage description.
   * @returns {Pargv}
   */
  Pargv.prototype.usage = function usage(_usage) {
    this.cli.usage = _usage || '';
    return this;
  };

  /**
   * Action
   * Adds an action to be called when
   * the given command is detected.
   * @param {string} cmd the command to match
   * @param {function} fn the callback to be called upon cmd match.
   * @returns {Command}
   */
  Pargv.prototype.command = function command(cmd, desc) {
    const command = new Command(cmd, desc, this);
    this.cli.commands = this.cli.commands || {};
    this.cli.commands[cmd] = command;
    return command;
  };
   ...
   * Displays help.
   */
  Pargv.prototype.help = function help() {
    return this;
  };
